...
五、
java -jar auth.jar server auth.yml
nohup java -jar auth.jar server auth.yml >/dev/null &
nohup java -Xms4096m -Xmx4096m -jar auth.jar server auth.yml >/dev/null &
curl -H "Content-type: application/json" -X POST -d '{"devSn":"add","devKey":"abc"}' http://10.213.142.63:8080/auth/gw/active
